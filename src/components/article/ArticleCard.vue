<template>
  <div class="group relative flex flex-col gap-3 md:gap-4 p-0 transition-all duration-300">
    ...
    <div class="flex flex-col gap-2 md:gap-3 mt-2 pr-2 md:pr-4">
      <div class="flex items-center gap-2 md:gap-3">
         <span class="text-[9px] md:text-[10px] font-bold text-zen-accent-light dark:text-zen-accent-dark uppercase tracking-widest">{{ article.category }}</span>
         <span class="w-1 h-1 rounded-full bg-gray-300 dark:bg-zinc-700"></span>
         <span class="text-[9px] md:text-[10px] text-gray-400 uppercase tracking-widest">{{ new Date(article.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) }}</span>
      </div>

      <h3 class="text-xl md:text-2xl font-serif font-bold text-zen-text-light dark:text-zen-text-dark leading-tight group-hover:text-zen-accent-light dark:group-hover:text-zen-accent-dark transition-colors">
